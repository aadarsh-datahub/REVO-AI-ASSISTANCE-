...
class OpenRouterClient:
    ...
    def _mark_rate_limited(self, model: str) -> None:
        ...
        logger.warning(
            f"[OpenRouter] Rate limited: {model} â€” "
    ...
    def _call(
        ...
        for attempt in range(1, MAX_RETRIES_PER_MODEL + 1):
            try:
                ...
                logger.warning(
                    f"[OpenRouter] {model} â†’ HTTP {resp.status_code} "
                    f"(attempt {attempt}/{MAX_RETRIES_PER_MODEL})"
                )

            except requests.exceptions.Timeout:
                logger.warning(
                    f"[OpenRouter] {model} â†’ Timeout "
                    f"(attempt {attempt}/{MAX_RETRIES_PER_MODEL})"
                )
            except Exception as e:
                logger.error(f"[OpenRouter] {model} â†’ Unexpected error: {e}")
    ...
    def _call_with_fallback(
        ...
        for m in pool:
            ...
            if result:
                logger.info(f"[OpenRouter] âœ“ Success: {m}")
                return result
    ...
    def chat(
        ...
        prompt: str,
        system: str = (
            "You are a component of MARK XXV, an AI assistant inspired by REVO. "
...
if __name__ == "__main__":
    print("=" * 55)
    print("  MARK XXV â€” OpenRouter Client Self-Test")
    print("=" * 55)
